...
export default function LinkComp({ data }: { data: link }) {
  ...
  const { deleteDoc } = useLinkContext() as linkContext;
  return (
    <div className="grid grid-cols-2 gap-3 justify-between p-5 text-lg md:text-xl border border-cyan-500 rounded-lg">
      ...
      <Link
        href={link}
        className="underline italic text-sm md:text-base text-indigo-300 col-span-2 place-self-start mt-3 hover:text-indigo-500"
        target="_blank"
      >
        {link}
      </Link>
